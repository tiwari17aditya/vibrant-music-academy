/**
 * Centralized Media Assets & External URLs Configuration
 * 
 * Developer Note: Edit this file to update any images, video embeds, social handles, 
 * studio map links, or external media resources across the entire application.
 */

export const SOCIAL_LINKS = {
  instagram: {
    handle: "@iyersir_vibrant_music",
    url: "https://instagram.com/iyersir_vibrant_music"
  },
  youtube: {
    handle: "@IyerSirMusicAcademy",
    url: "https://youtube.com/@IyerSirMusicAcademy"
  },
  whatsapp: {
    number: "+919820012345",
    directUrl: "https://wa.me/919820012345"
  }
};

export const IMAGE_ASSETS = {
  // Hero section main acoustic guitar image
  heroBanner: "https://images.unsplash.com/photo-1511671782779-c97d3d27a1d4?w=800&auto=format&fit=crop&q=80",
  
  // Studio & practice classroom photos
  studioClassroom: "https://images.unsplash.com/photo-1525201548942-d8732f6617a0?w=800&auto=format&fit=crop&q=80",
  
  // Instagram Reels Thumbnail Mockups
  reelsThumbnails: {
    reel1: "https://images.unsplash.com/photo-1511671782779-c97d3d27a1d4?w=600&auto=format&fit=crop&q=80",
    reel2: "https://images.unsplash.com/photo-1525201548942-d8732f6617a0?w=600&auto=format&fit=crop&q=80",
    reel3: "https://images.unsplash.com/photo-1465847899084-d164df4dedc6?w=600&auto=format&fit=crop&q=80",
    reel4: "https://images.unsplash.com/photo-1507838153414-b4b713384a76?w=600&auto=format&fit=crop&q=80"
  },

  // YouTube Tutorial Video Cover Thumbnails
  youtubeThumbnails: {
    lesson1: "https://images.unsplash.com/photo-1511671782779-c97d3d27a1d4?w=800&auto=format&fit=crop&q=80",
    lesson2: "https://images.unsplash.com/photo-1507838153414-b4b713384a76?w=800&auto=format&fit=crop&q=80"
  }
};

export const YOUTUBE_EMBED_URLS = {
  guitarFoundationsMasterclass: "https://www.youtube.com/embed/dQw4w9WgXcQ",
  trinityGradePrepTutorial: "https://www.youtube.com/embed/dQw4w9WgXcQ"
};

export const LOCATION_ASSETS = {
  studioAddress: "Ghansoli, Navi Mumbai, Maharashtra 400701",
  // Google Maps Embed Iframe URL (Optional replace with actual studio Google Maps link)
  googleMapsEmbedUrl: "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3770.187313627918!2d73.0039!3d19.1256!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3be7c0c1b7555555%3A0x123456789abcdef!2sGhansoli%2C%20Navi%20Mumbai!5e0!3m2!1sen!2sin!4v1700000000000"
};
